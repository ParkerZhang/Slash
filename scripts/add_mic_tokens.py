...
def main():
    if len(sys.argv) < 2:
        print('Usage: add_mic_tokens.py <tokens_json_path>', file=sys.stderr)
        sys.exit(1)
    ...
    for token in tokens:
        ...
        for variant in {v, v.lower()}:
            if variant not in vocab:
                ...
                new_tokens.append((variant, next_id))
                next_id += 1
    ...
    for i, init in enumerate(model.graph.initializer):
        ...
        print('Error: embeddings.word_embeddings.weight not found', file=sys.stderr)
        sys.exit(1)
