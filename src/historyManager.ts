...
export function addLoadedFile(id: string, name: string, keyField?: string): void {
    const history = loadHistory();
    const existing = history.loadedFiles.findIndex(f => f.id === id);
    if (existing >= 0) {
        history.loadedFiles[existing] = { id, name, keyField: keyField || history.loadedFiles[existing].keyField };
    } else {
        history.loadedFiles.push({ id, name, keyField });
    }
    saveHistory(history);
}
