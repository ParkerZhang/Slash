...
# TICKER Fingerprint Strengths
TICKER_STRENGTH = 0.45
MIC_BIAS = 0.15
ISIN_BIAS = 0.10
SEDOL_BIAS = 0.10
...
def main():
    ...
    for t in ticker_data:
        ...
        if t_low not in vocab:
            vocab[t_low] = max(int(v) for v in vocab.values()) + 1
            added += 1
    if added:
        with open(TOK_PATH, 'w') as f:
            json.dump(tok, f, indent=2, ensure_ascii=False)
        print(f"Added {added} TICKER tokens.")
    ...
    for ticker, info in ticker_data.items():
        ...
        # D. Composite Fingerprint [MIC, ISIN, SEDOL, TICKER]
        final_emb = np.zeros(384, dtype=np.float32)
        final_emb[VAC_DIMS] = [MIC_BIAS, ISIN_BIAS, SEDOL_BIAS, TICKER_STRENGTH]
        final_emb += 1.0 * semantic_shell

        tid = vocab.get(ticker.lower())
        if tid: W[tid] = final_emb

        print(f"TICKER: {ticker.upper():10} -> {info['name']} (Quad-Anchored)")

    # 5. Save
    new_init = numpy_helper.from_array(W, name=target_init.name)
    model.graph.initializer[init_idx].CopyFrom(new_init)
    onnx.save(model, ONNX_PATH)
    print("\nModel updated with quad-anchored TICKER embeddings.")
